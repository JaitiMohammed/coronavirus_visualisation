...
tickFont = {'size': 12,
            'color': "rgb(30,30,30)", 'family': "Courier New, monospace"}
...
def update_states(country):
    states = list(allData.loc[allData['Country/Region']
                              == country]['Province/State'].unique())
    states.insert(0, '<all>')
    states.sort()
    ...
    figure = go.Figure(data=[
        go.Bar(
            ...
            marker_color={'Deaths': 'rgb(200,30,30)', 'Recovered': 'rgb(30,200,30)',
                          'Confirmed': 'rgb(100,140,240)'}[metric]
...
@app.callback(
    ...
    [Input('country', 'value'), Input(
        'state', 'value'), Input('metrics', 'value')]
)
